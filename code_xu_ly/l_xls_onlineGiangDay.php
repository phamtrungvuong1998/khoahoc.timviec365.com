...
if ($search == '') {
    $db_course = new db_query("SELECT * FROM courses Where user_id = '$user_id' AND course_type = 2");
    $html = '';
    ...
    while ($row = mysql_fetch_assoc($db_course->result)) {
        ...
        if ($row['price_listed'] == -1) {
            $price_listed = "Chưa cập nhật";
        }else{
            $price_listed = number_format($row['price_listed']) . " đ";
        }
        //print_r($row);
        $html .= '<tr>
        ...
        $html .= '<td><center>' . $price_listed . '</center></td>';
    ...
    $db_course = new db_query("SELECT * FROM courses Where user_id = '$user_id' AND course_type = 2 AND course_name LIKE '%$search%'");
    $html = '';
    $html .= '<table border="1" cellspacing="1">
            <tr>
            <th>STT</th>
...
header("Content-Disposition: attachment; filename=online-giangday.xls");
