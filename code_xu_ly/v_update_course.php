...
if (strtotime(date("d-m-Y")) - $row24['created_at'] >= 86400) {
	$data24 = [
		'24_course'=>0
	];
	...
	if ($row['24_course'] == 24) {
		...
		if ($submit == 1) {
			...
			if (mysql_num_rows($qr_course_slug->result) > 0) {
				...
				$qrTeach = new db_query("SELECT course_id FROM user_center_teacher WHERE center_teacher_id = $teach");
				$rowTeach = mysql_fetch_array($qrTeach->result);
				if ($rowTeach['course_id'] == 0) {
					$dataTeach = [
						'course_id'=>$course_id
					];
					...
					update('user_center_teacher',$dataTeach,$dataId);
				}else{
					$arr_course_id = explode(",", $rowTeach['course_id']);
					$arr_course_id[count($arr_course_id)] = $course_id;
					$teach_course_id = implode(",", $arr_course_id);
					$dataTeach = [
						'course_id'=>$teach_course_id
					];
			...
			if (mysql_num_rows($qr_course_slug->result) > 0) {
				...
				$_FILES['file']['name'] = md5(rand()) . "." . $duoi;
				move_uploaded_file($_FILES['file']['tmp_name'], '../img/course/'.$_FILES['file']['name']);
				setcookie('course_avatar',$_FILES['file']['name'],time() + 900, "/");
				...
				$level = getValue('level','int','POST','');
				setcookie('level',$level,time() + 900, "/");

				$tag_id = getValue('tag_id','int','POST','');
				setcookie('tag_id',$tag_id,time() + 900, "/");
				$type = setcookie('type', 3, time() + 900, '/');
				$advantages_id = getValue('advantages_id','arr','POST','');
				if ($advantages_id == "") {
					$advantages = 0;
				}else{
