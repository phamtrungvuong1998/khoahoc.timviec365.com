...
if ($type == 0) {
  if ($arr != "") {
    if (isset($arr['tag_id'])) {
      ...
      $qrCount = new db_query("SELECT course_id FROM courses WHERE course_type = 2 AND tag_id = '$tag_id' AND price_listed > -1 AND courses.hide_course = 1 AND courses.accept = 1");
    }else if (isset($arr['keyword'])) {
      $key = $arr['keyword'];
      $qrCourse = new db_query("SELECT * FROM courses JOIN levels ON courses.level_id = levels.level_id INNER JOIN categories ON courses.cate_id = categories.cate_id INNER JOIN users ON users.user_id = courses.user_id WHERE courses.course_type = 2 AND courses.course_name LIKE '%$key%' AND price_listed > -1 AND courses.hide_course = 1 AND courses.accept = 1 ORDER BY courses.course_id DESC LIMIT $start, $end");
      $qrCount = new db_query("SELECT course_id FROM courses WHERE course_type = 2 AND course_name LIKE '%$key%' AND price_listed > -1 AND courses.hide_course = 1 AND courses.accept = 1");
      ...
      $qrCount = new db_query("SELECT course_id FROM courses WHERE course_type = 2 AND cate_id = $cate_id AND price_listed > -1 AND courses.hide_course = 1 AND courses.accept = 1");
    }
  }else{
    $qr = "SELECT * FROM courses INNER JOIN levels ON courses.level_id = levels.level_id INNER JOIN categories ON courses.cate_id = categories.cate_id INNER JOIN users ON courses.user_id = users.user_id WHERE courses.course_type = 2 AND courses.price_listed > -1 AND courses.hide_course = 1 AND courses.accept = 1 ". $qrPrices . "  LIMIT $start,$end";
    $qrCourse = new db_query($qr);
    $qrCount = new db_query("SELECT * FROM courses WHERE course_type = 2 AND price_listed > -1 AND courses.hide_course = 1 AND courses.accept = 1");
  }
}else if ($type == 1){
  $qr ="SELECT * FROM courses INNER JOIN levels ON courses.level_id = levels.level_id INNER JOIN categories ON courses.cate_id = categories.cate_id INNER JOIN users ON courses.user_id = users.user_id WHERE courses.course_type = 2 AND courses.cate_id = '$cate_id' AND price_listed > -1 AND courses.hide_course = 1 AND courses.accept = 1 ". $qrPrices. "  LIMIT $start,$end";
  $qrCourse = new db_query($qr);
  ...
  $qrCourse = new db_query($qr);
  $qrCount = new db_query("SELECT course_id FROM courses INNER JOIN tags ON courses.tag_id = tags.tag_id WHERE course_type = 2 AND courses.tag_id = '$tag_id' AND price_listed > -1 AND courses.hide_course = 1 AND courses.accept = 1");
}else if ($type == 3) {
  $qr = "SELECT * FROM courses INNER JOIN levels ON courses.level_id = levels.level_id INNER JOIN categories ON courses.cate_id = categories.cate_id INNER JOIN users ON courses.user_id = users.user_id WHERE courses.course_type = 2 AND courses.user_id = $teacher_id AND courses.cate_id = $cate_id AND price_listed > -1 AND courses.hide_course = 1 AND courses.accept = 1 ". $qrPrices ."  LIMIT $start,$end";
  $qrCourse = new db_query($qr);
  $qrCount = new db_query("SELECT course_id FROM courses WHERE course_type = 2 AND user_id = '$teacher_id' AND cate_id = '$cate_id' AND price_listed > -1 AND courses.hide_course = 1 AND courses.accept = 1");
}else if ($type == 4) {
 $qr = "SELECT * FROM courses INNER JOIN levels ON courses.level_id = levels.level_id INNER JOIN categories ON courses.cate_id = categories.cate_id INNER JOIN users ON courses.user_id = users.user_id INNER JOIN tags ON courses.tag_id = tags.tag_id WHERE courses.course_type = 2 AND courses.user_id = $teacher_id AND courses.cate_id = $cate_id AND courses.tag_id = '$tag_id' AND price_listed > -1 AND courses.hide_course = 1 AND courses.accept = 1 ". $qrPrices ."  LIMIT $start,$end";
 $qrCourse = new db_query($qr);

 $qrCount = new db_query("SELECT course_id FROM courses INNER JOIN tags ON courses.tag_id = tags.tag_id WHERE course_type = 2 AND courses.user_id = '$teacher_id' AND courses.cate_id = '$cate_id' AND courses.tag_id = '$tag_id' AND price_listed > -1 AND courses.hide_course = 1 AND courses.accept = 1");
}else if ($type == 5) {
  $qr = "SELECT * FROM courses INNER JOIN levels ON courses.level_id = levels.level_id INNER JOIN categories ON courses.cate_id = categories.cate_id INNER JOIN users ON courses.user_id = users.user_id WHERE courses.course_type = 2 AND courses.user_id = $teacher_id AND price_listed > -1 AND courses.hide_course = 1 AND courses.accept = 1 ". $qrPrices ."  LIMIT $start,$end";
  $qrCourse = new db_query($qr);
  $qrCount = new db_query("SELECT course_id FROM courses WHERE course_type = 2 AND user_id = $teacher_id AND price_listed > -1 AND courses.hide_course = 1 AND courses.accept = 1");
}
...
<a href="'.urlDetail_courseOnline($row['course_id'],$row['course_slug']).'"><img class="img-main" onerror=\'this.onerror=null;this.src="../img/avatar/error.png";\'
...
<div class="prd-name">
<a
href="'.urlDetail_courseOnline($row['course_id'],$row['course_slug']).'">
