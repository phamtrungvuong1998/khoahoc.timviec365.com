...
// var_dump($_GET);
if ($search != '') {
    $db_count = new db_query("SELECT count(order_id) as total FROM `orders` INNER JOIN users ON orders.user_student_id = users.user_id INNER JOIN courses ON orders.course_id = courses.course_id WHERE orders.course_type = 2 AND courses.user_id = '9' AND (orders.order_id LIKE '%$search%' OR courses.course_name LIKE '%$search%')");
    ...
    if ($total_records <= 0) {
        ...
        $db_point = new db_query("SELECT * FROM `orders` INNER JOIN users ON orders.user_student_id = users.user_id INNER JOIN courses ON orders.course_id = courses.course_id WHERE orders.course_type = 2 AND courses.user_id = '$user_id' AND (orders.order_id LIKE '%$search%' OR courses.course_name LIKE '%$search%') ORDER BY order_id DESC LIMIT $start,$limit");
        $html = '';
        $arr = [];
        echo '<div class="l_excel"><a href="../code_xu_ly/l_xls_onlinedaban.php?search=' . $search . '"><button class="l_btn_excel">
        ...
        while ($row = mysql_fetch_assoc($db_point->result)) {
            ...
            $html .= '
                        <div class="l_table-cell l_curson">
                            <div class="l_curson" onclick="l_chinhsua(' . $row['order_id'] . ')">
                                <img src="../img/More.svg" alt="loading...">
                            </div>
                            <div class="l_hienthi_chinhsua" id="l_hienthi_chinhsua' . $row['order_id'] . '">
