...
// var_dump($_GET);
if ($search != '') {
    $db_count = new db_query("SELECT COUNT(course_id) as total FROM courses Where course_type=1 AND user_id ='$user_id' AND course_name LIKE '%$search%'");
    ...
    if ($total_records <= 0) {
        ...
        $db_point = new db_query("SELECT * FROM courses Where course_type=1 AND user_id ='$user_id' AND course_name LIKE '%$search%' ORDER BY course_id DESC LIMIT $start,$limit");
        $html = '';
        $arr = [];
        echo '<div class="l_excel"><a href="../code_xu_ly/l_xls_offlineGiangDay.php?search=' . $search . '"><button class="l_btn_excel">
