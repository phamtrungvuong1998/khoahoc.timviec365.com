...
$qr_count = new db_query("SELECT courses.course_id FROM courses INNER JOIN course_basis ON courses.course_id = course_basis.course_id INNER JOIN city ON city.cit_id = course_basis.cit_id WHERE courses.course_type = 1" . $qr_id . $qr_cate_id . $qr_tag_id . $qr_teacher_id . $qr_address);
...
while($rowHV = mysql_fetch_array($qr->result)){
	...
	<div class="v_title_student">'.number_format($rowHV['price_listed']) . ' đ'.'</div>
	...
	id="v_accept'.$rowHV['course_id'].'" data-course="'.$rowHV['course_id'].'" name="student_accept"
	onclick="accept_course(this)" '.$user_accept.'></div>
	<div class="v_title_student"><input type="checkbox" class="v_index"
	id="v_index'.$rowHV['course_id'].'" name="student_index"
	onclick="active('.$rowHV['course_id'].')" '.$user_index.'></div>
