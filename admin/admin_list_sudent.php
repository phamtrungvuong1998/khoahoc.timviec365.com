...
<body>
    ...
    <div class="templatemo-flex-row">
        ...
        <div class="templatemo-content col-1 light-gray-bg">
            ...
            $qrUserId = new db_query("SELECT user_name FROM users WHERE user_type = 1 ORDER BY user_id DESC");
            $qrPhone = new db_query("SELECT user_id,user_phone FROM users WHERE user_type = 1 ORDER BY user_id DESC");
            $qrEmail = new db_query("SELECT user_id,user_mail FROM users WHERE user_type = 1 ORDER BY user_id DESC");
            ?>
            <div id="v_filter_date">
                <span>Từ:</span>
                <input type="date" id="startTime" onchange="v_filter_student()">
                <span>Đến:</span>
                <input type="date" id="endTime" onchange="v_filter_student()">
            </div>
            <div id="v_filter">
                <div class="v_filter">
                ...
                <div class="v_filter">
                    <span>Học viên:</span>
                    <select name="" id="v_name" class="v_filter_select" onchange="v_filter_student()">
                ...
                <div class="v_filter">
                    ...
                    <select name="" id="v_phone" class="v_filter_select" onchange="v_filter_student()">
                ...
                <div class="v_filter">
                    <span>Email:</span>
                    <select name="" id="v_email" class="v_filter_select" onchange="v_filter_student()">
            ...
            <div class="v_filter">
                <a href="../code_xu_ly/v_admin_xls.php?type=1" id="v_href_exel"><button id="v_xls">XUẤT
                        EXCEL</button></a>
            </div>
            <center id="v_info_ad">
                <div class="title_input" id="title_manager">
                    <div id="manager">
                        ...
                        <div class="v_title_student">Mã học viên</div>
                        ...
                        <div class="v_title_student">quận huyện</div>
                        <div class="v_title_student">Tỉnh, thành phố</div>
                    ...
                    <?php 
                    $i = 1;
                    $qr = new db_query("SELECT * FROM users INNER JOIN city ON users.cit_id = city.cit_id WHERE user_type = 1 ORDER BY user_id DESC LIMIT 0, 30");
                    $count = new db_query("SELECT * FROM users WHERE user_type = 1");
                    ...
                    <div class="manager" id="manager<?php echo $rowHV['user_id'];?>">
                        ...
                        <div class="v_title_student"><a
                                href="<?=urlDetail_student($rowHV['user_id'],$rowHV['user_slug'])?>"><?php echo $rowHV['user_name']; ?></a>
                        </div>
                        ...
                        <div class="v_title_student"><?php echo $district; ?></div>
                        <div class="v_title_student"><?php echo $city; ?></div>
                        ...
                        if ($_COOKIE['adm_type'] == 0) {
                            ...
                            if (mysql_num_rows($check->result) > 0) {
                                ...
                                src="../img/vv_edi.svg" onclick="v_student_edit('. $rowHV['user_id'].')"
                                alt="Ảnh lỗi"></div>
                                ';
                            }
                        }else{
...
<script src="../js/v_admin.js"></script>
...
function v_filter_student() {
    ...
    $("#v_href_exel").attr({
        'href': '../code_xu_ly/v_admin_xls.php?type=1' + get_id + get_name + get_phone + get_email +
            get_startTime + get_endTime
    });
    $.ajax({
        url: '../ajax/v_admin_list_student.php',
        type: 'POST',
        dataType: 'json',
        data: {
            user_id: user_id,
...
function v_student_edit(user_id) {
    $.ajax({
        url: '../ajax/Admin_update_student.php',
        type: 'POST',
        dataType: 'json',
        data: {
            student_id: user_id
...
function active(user_id) {
    ...
    $.ajax({
        ...
        error: function () {
            ...
            if ($("#v_active" + user_id)[0].checked === true) {
                $("#v_active" + user_id)[0].checked = false;
            } else if ($("#v_active" + user_id)[0].checked === false) {
                $("#v_active" + user_id)[0].checked = true;
            }

            if ($("#v_index" + user_id)[0].checked === true && $("#v_index" + user_id)[0].disabled === false) {
