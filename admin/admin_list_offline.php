...
<body>
    ...
    <div class="templatemo-flex-row">
        ...
        <div class="templatemo-content col-1 light-gray-bg">
            ...
            <?php
            $qrId = new db_query("SELECT course_id FROM courses WHERE course_type = 1 ORDER BY course_id DESC");
            ...
            <div id="v_filter">
                ...
                <div class="v_filter">
                    <span>Địa điểm:</span>
                    <select name="" id="v_address" class="v_filter_select" onchange="v_filter_student()">
                        <option value="0">Địa điểm</option>
                        <?php while($rowCity = mysql_fetch_array($qrCit->result)){ ?>
                        <option value="<?=$rowCity['cit_id']?>"><?=$rowCity['cit_name']?></option>
                        <?php } ?>
                    </select>
                </div>
            </div>
            ...
            <center id="v_info_ad">
                <div class="title_input" id="title_manager">
                    <div id="manager">
                        ...
                        <div class="v_title_student">Địa điểm</div>
                        <div class="v_title_student">Giá gốc</div>
                        <div class="v_title_student">Giá khuyến mại</div>
                        <div class="v_title_student">Ngày tạo</div>
                        <div class="v_title_student">Ngày cập nhật</div>
                        <div class="v_title_student">Kiểm duyệt</div>
                        <div class="v_title_student">index</div>
                        <div class="v_title_student">Sửa</div>
                    </div>
                    <?php 
                    $i = 1;
                    $qr = new db_query("SELECT * FROM courses INNER JOIN categories ON courses.cate_id = categories.cate_id INNER JOIN users ON users.user_id = courses.user_id INNER JOIN course_basis ON courses.course_id = course_basis.course_id INNER JOIN city ON city.cit_id = course_basis.cit_id WHERE courses.course_type = 1 AND courses.hide_course = 1 ORDER BY courses.course_id DESC LIMIT 0, 30");
                    ...
                    <div class="manager" id="manager<?php echo $rowHV['course_id'];?>">
                        ...
                        <div class="v_title_student"><?php echo number_format($rowHV['price_listed']) . " đ"; ?></div>
                        <div class="v_title_student"><?php
                        if ($rowHV['price_promotional'] == 'false') {
                            echo 'Chưa cập nhật';
                        }else{
                            echo number_format($rowHV['price_promotional']) . " đ";
                        } 
                         ?>
                        </div>
                        ...
                        <div class="v_title_student"><input type="checkbox" class="v_index"
                                id="v_accept<?php echo $rowHV['course_id']; ?>" data-course="<?php echo $rowHV['course_id']; ?>" name="student_accept"
                                onclick="accept_course(this)" <?php echo $user_accept; ?>></div>
                        <div class="v_title_student"><input type="checkbox" class="v_index"
                                id="v_index<?php echo $rowHV['course_id']; ?>" name="student_index"
                                onclick="active(<?php echo $rowHV['course_id']; ?>)" <?php echo $user_index; ?>></div>
...
function v_filter_student() {
    ...
    if (address == 0) {
        ...
        var get_address = '&address=' + address;
    }

    $("#v_href_exel").attr({
        'href': '../code_xu_ly/v_admin_offline_xls.php?type=1&' + get_id + get_cate + get_tag + get_teacher +
            get_prices + get_address
    });
    $.ajax({
        url: '../ajax/v_admin_list_offline.php',
        type: 'POST',
        dataType: 'json',
        data: {
            course_id: course_id,
...
function v_student_edit(course_id, teacher_center) {
    ...
    if (adm_edit == 0) {
        ...
        if (teacher_center == 3) {
            var urls = '../ajax/Admin_update_offline.php';
        } else if (teacher_center == 2) {
            var urls = '../ajax/Admin_update_offline2.php';
        }
        $.ajax({
            url: urls,
            type: 'GET',
...
function validate_update_student(course_id) {
    if ($("#center_teacher_id").val() == undefined) {
        var center_teacher_id = 0;
    } else {
        var center_teacher_id = $("#center_teacher_id").val();
    }
    $.ajax({
        url: '../code_xu_ly/v_admin_update_offline.php',
        type: 'POST',
        dataType: 'json',
        data: {
            course_id: course_id,
            course_describe: $("#course_describe").val(),
            course_learn: $("#course_learn").val(),
            course_object: $("#course_object").val(),
            course_request: $("#course_request").val(),
            center_teacher_id: center_teacher_id,
            time_learn: $("#time_learn").val(),
            course_slide: $("#course_slide").val(),
            price_listed: $("#price_listed").val(),
            price_promotional: $("#price_promotional").val(),
            quantity_std: $("#quantity_std").val(),
            price_discount: $("#price_discount").val(),
            certification: document.course_form.certification.value,
            course_level: document.course_form.course_level.value,
            month_study: $("#month_study").val()
        },
        success: function(data) {
            window.location.href = '/admin/admin_list_offline.php';
        }
    });
...
function active(user_id) {
    ...
    $.ajax({
        ...
        success: function (data) {
            $("#v_index" + user_id)[0].disabled = true;
        },
        error: function () {
            alert("Có lỗi xảy ra. Vui lòng thử lại");
            if ($("#v_index" + user_id)[0].checked === true) {
                $("#v_index" + user_id)[0].checked = false;
            } else if ($("#v_index" + user_id)[0].checked === false) {
                $("#v_index" + user_id)[0].checked = true;
            }
        }
    });
}
