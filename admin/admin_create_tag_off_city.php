                    ...
                    <div class="v_detail_student">
                        ...
                        <div>
                            <select name="tag_id" id="tags">
                                ...
                                <?
                                $db_cate = new db_query("SELECT * FROM tags");
                                while ($row = mysql_fetch_assoc($db_cate->result)) {
                                ?>
                                    <option value="<? echo $row['tag_id'] ?>"><? echo $row['tag_name'] ?></option>
                                <?
                                }
                                ?>
                            </select>
...
<script type="text/javascript">
    ...
    function l_validate() {
        ...
        if (tag == "") {
            document.getElementById("l_error4").innerHTML = "Chọn môn học chi tiết";
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.result == 1) {
                        alert('Tin môn học chi tiết tại tỉnh thành đã tồn tại');
                    } else if (response.result == 2) {
                        alert('Đăng tin thành công');
                    window.location.href = 'admin_list_tags_off_city.php';
                    }else {
