...
if ($tag_id == 0 || $type == 0) {
    header("location:admin_list_category.php");
}
if ($type == 2) {
    $db_tag = new db_query("SELECT * FROM categories WHERE cate_id = $tag_id");
    $row_tag = mysql_fetch_assoc($db_tag->result);
    $tag_name = $row_tag['cate_name'];
...
<script type="text/javascript">
    ...
    function validate_update_center(a, type) {
        ...
        if (flag == true) {
            ...
            $.ajax({
                url: '../ajax/admin_update_category.php',
                type: "post",
                data: data,
                dataType: 'json',
                contentType: false,
    ...
    // function validate_update_center(a, b) {
    //     // console.log('123123');
    //     var flag = l_validate();
    //     if (flag == true) {
    //         var user = document.getElementById('cate_name').value;
