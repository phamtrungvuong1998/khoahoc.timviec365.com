...
$qrCount = new db_query("SELECT COUNT(*) FROM  orders JOIN users ON users.user_id = orders.user_student_id JOIN courses ON courses.course_id = orders.course_id WHERE courses.user_id = $cookie_id AND orders.course_type = 1");
...
<head>
    ...
    #v_sidebar_tb-2 li:nth-child(4) a{
        color: #1B6AAB;
    }
    .v_popup {
        width: 160px;
            ...
            <div id="content-box" class="flex">
                ...
                <div id="content">
                        <div id="v_content-title">
                            ...
                            <div class="v_content-title-div">NGÀY ĐẶT CHỖ</div>
                            <div class="v_content-title-div">GIÁ GỐC</div>
                            <div class="v_content-title-div">GIÁ KHUYẾN MẠI</div>
                        </div>
                        <div id="filter">
                            <?
                            $qr1 = new db_query("SELECT *,users.user_id,users.user_slug FROM orders JOIN users ON users.user_id = orders.user_student_id JOIN courses ON courses.course_id = orders.course_id WHERE courses.user_id = $cookie_id AND orders.course_type = 1 LIMIT $start,$end");
                            ...
                            <div class="v_noidungkh" id="v_noidungkh-">
                                ...
                                <div class="v_content-list v_kh1"><a href="<?php echo urlDetail_courseOffline($row1['course_id'],$row1['course_slug']); ?>" class="v_kh"><?=$row1['course_name']?></a></div>
                                <div class="v_content-list v_trungtam">
                                    <a href="<?php echo urlDetail_student($row1['user_id'],$row['user_slug']); ?>" class="v_name-hv"><?=$row1['user_name']?></a>
                                </div>
                                <div class="v_content-list"><?=date("d-m-Y", $row1['day_buy'])?></div>
                                <div class="v_content-list"><?=number_format($row1['price_listed'])?></div>
                                <div class="v_content-list"><?php
                                if ($row1['price_promotional'] == 'false') {
                                    echo "Chưa cập nhật";
                                }else{
                ...
                <div id="filter2">
                    <?php 
                            $qr2 = new db_query("SELECT * FROM orders JOIN users ON users.user_id = orders.user_student_id JOIN courses ON courses.course_id = orders.course_id 
                                JOIN categories ON categories.cate_id = courses.cate_id WHERE courses.user_id = $cookie_id AND orders.course_type = 1 LIMIT $start,$end");
                    ...
                    <div class="v_content-mb">
                        ...
                        <div class="flex v_info-all v_content-mb-div">
                            ...
                            <div class="v_content-mb-thongtin"><span class="v_content-mb-span">Tên học viên :</span>
                                <?=$row2['user_name']?></div>
                            <div class="v_content-mb-thongtin"><span class="v_content-mb-span">Ngày đặt chỗ
                                </span><?=date("d-m-Y", $row2['day_buy'])?></div>
                            ...
                            <div class="v_content-mb-thongtin"><span class="v_content-mb-span">Giá khuyến mại :</span>
                                <?php
                                if ($row2['price_promotional'] == 'false') {
                                    echo "Chưa cập nhật";
                                }else{
                ...
                <div id="v_chuyentrang">
                    <a class="v_chuyen-trang-div" href="<?php if($p == 1){
                        echo '/giang-vien-khoa-hoc-offline-da-dat-cho/id' . $_COOKIE['user_id'] . '-p1.html';
                        }else{
                            echo '/giang-vien-khoa-hoc-offline-da-dat-cho/id' . $_COOKIE['user_id'] . '-p' . ($p-1).'.html';
                        } ?>">&lt;</a>
                    <?php for ($i = 0; $i  < $pa ; $i++) { ?>
                    <a href="/giang-vien-khoa-hoc-offline-da-dat-cho/id<?php echo $_COOKIE['user_id']; ?>-p<?php echo $i  + 1; ?>.html"
                        class="v_chuyen-trang-div <?php if($p == $i+1){
                                    echo "p_active";
                                } ?>" class="v_tranght"><?php echo $i + 1; ?></a>
                    <?php } ?>
                    <a href="<?php if($p == $i){
                                echo '/giang-vien-khoa-hoc-offline-da-dat-cho/id' . $_COOKIE['user_id'] . '-p'. ($i) .'.html';
                                }else{
                                    echo '/giang-vien-khoa-hoc-offline-da-dat-cho/id' . $_COOKIE['user_id'] . '-p' . ($p+1).'.html';
